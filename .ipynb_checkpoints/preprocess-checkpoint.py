...
def load_data(dataset):
	...
	if dataset == 'STRAIN':
	    ...
	    # Convert train and test data to numpy arrays and normalize
	    ...
	    for i, label_col in enumerate(labels_df.columns):
	        labels = labels_df[label_col].values.astype(float).reshape(-1, 1)
	        
	        # Save the labels, train, and test datasets for each label column
	        np.save(os.path.join(dataset_folder, f'train_{i}.npy'), train)
	        np.save(os.path.join(dataset_folder, f'test_{i}.npy'), test)
	        np.save(os.path.join(dataset_folder, f'labels_{i}.npy'), labels)
	        
	        print(f'Processed label column: {label_col}')
	        print(train)
